...
enable Booth and block4 to benefit FP verification. Fourth, the
observed correlation between carry propagation and SAT hardness
invites two follow-up studies: (i)~a controlled experiment with
encoding variants that independently vary carry depth, XOR
structure, and variable dependency (a detailed design is included
with the artifact); and (ii)~a formal proof that integer
multiplication commutativity requires exponential-size resolution
proofs on standard encodings (a research plan is also included).
Either result would upgrade Section~\ref{sec:hardness}'s
correlation claim into a causation claim.
