...
then
  ...
  then
    ...
    cmd='${script_folder}/cpplint.py --exclude=jbmc/src/miniz/* --exclude=scripts/check_irep_moves.cpp --filter=-whitespace/operators,-readability/identifier_spacing $file 2>&1 >/dev/null'
  fi
else
