#!/usr/bin/env python3
"""
N3 Phase 3 full proof using cut_v2 BP (Cut(row)-merging BP).

Adapted from phase3_full.py but calls build_bp_cut_rows instead of
build_bp, and emit_tree_post_order_cut instead of v9's.
"""
...
def compose_full_proof_cut(n, out_cnf, out_drat):
    """Emit full CNF and composed DRAT proof using cut_v2 BP."""
    ...
    with open(out_drat, "w") as out:
        ...
        for k in range(0, 2*n):
            ...
            if bp.get("conflict") is not None:
                # Trivial strip: emit weakened empty.
                ww.write("0")
            else:
...
def main():
    if len(sys.argv) != 2:
        print("usage: phase3_full_cut.py N", file=sys.stderr)
        sys.exit(1)
    n = int(sys.argv[1])

    cnf_path = f"/tmp/phase3_full_cut_n{n}.cnf"
    drat_path = f"/tmp/phase3_full_cut_n{n}.drat"

    strip_sizes = compose_full_proof_cut(n, cnf_path, drat_path)
