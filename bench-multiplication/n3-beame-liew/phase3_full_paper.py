#!/usr/bin/env python3
"""
N3 Phase 3 full proof using paper-exact tree BP.
Analogous to phase3_full_diag.py but uses phase3_bp_paper.
"""
...
from phase3_bp_paper_drat import emit_tree_post_order


def compose_full_proof_paper(n, out_cnf, out_drat):
    # Use tree BP (MERGE=False) for valid DRAT emission.
    phase3_bp_paper.MERGE_NODES = False
    ...
    with open(out_drat, "w") as out:
        ...
        for k in range(0, 2*n):
            _, strip_clauses_k, _, _, _, _ = build_bp_paper(n, k)[:6]
            # Actually build_bp_paper returns (cnf, strip_clauses, bp, role2var, out_c, out_d)
            _, _, bp, _, _, _ = build_bp_paper(n, k)
            ...
            class WeakenedWriter:
                ...
                def write(self, s):
                    for line in s.split("\n"):
                        ...
                        sorted_lits = sorted(
                            weakened, key=lambda x: (abs(x), x)
                        )
                        if sorted_lits:
                            self.f.write(
                                " ".join(str(l) for l in sorted_lits) + " 0\n"
                            )
                        else:
                            self.f.write("0\n")
                        self.count += 1
        ...
        # Final: contradiction via diff chain
...
def main():
    if len(sys.argv) != 2:
        print("usage: phase3_full_paper.py N", file=sys.stderr)
        sys.exit(1)
    n = int(sys.argv[1])
    cnf_path = f"/tmp/phase3_full_paper_n{n}.cnf"
    drat_path = f"/tmp/phase3_full_paper_n{n}.drat"
    strip_sizes = compose_full_proof_paper(n, cnf_path, drat_path)
