// A CLASS-type tuple element (box<int>) exercises the aggregate
// [class.base.init]/7 late lowering for a NON-POD aggregate base
// (__tuple_leaf<box<int>> is an aggregate, [dcl.init.aggr]/1, though
// not a POD) and the [expr.sizeof]/5 pack-size queries surviving the
// single-element pack-name stamping during instantiation
// (`sizeof...(_Tp)` must stay a pack-size query, not become the
// element's byte size).  clang accepts; g++ rejects the
// alias-template pack deduction shape.
...
using __apply_cv_t = __apply_cv_impl::__apply<_Up>;
template <class _Fp, class... _Args>
decltype(_Fp()(declval<_Args>()...)) __invoke(_Fp __f, _Args &&...__args)
{
  return __f(__args...);
}
template <class, class _Fp, class... _Args>
...
struct
{
  ...
  auto operator()(_Tp (&__t)[_Np])
  {
    return __t;
  }
} begin;
template <class _Tp>
using iterator_t = decltype(begin(declval<_Tp>()));
...
using __make_indices_imp =
  __make_integer_seq<__integer_sequence, long, _Ep - _Sp>::
...
template <class _Tp>
constexpr long tuple_size_v = tuple_size<_Tp>::value;
template <class T>
struct box
{
  T v;
...
int main()
{
  std::tuple<takeish, std::box<int>> t(takeish(), std::box<int>(3));
  __CPROVER_assert(std::get<1>(t).v == 3, "nested box elem");
  return 0;
}
