...
struct _Head_base
{
  static int _M_head(_Head_base __b)
  {
...
struct _Tuple_impl<_Idx, _Head> : _Head_base
{
  ...
  _Tuple_impl(_UHead __head) : _Head_base(__head)
  {
  }
};
...
template <typename _Tp>
struct allocator_traits<__new_allocator<_Tp>>
{
  using allocator_type = __new_allocator<_Tp>;
  template <typename _Up>
  using rebind_alloc = __new_allocator<_Up>;
...
_Rb_tree_iterator lower_bound()
{
  return _M_lower_bound(
    static_cast<_Link_type>(_M_header._M_parent), &_M_header);
}
struct
{
  template <typename... _Args>
  auto _M_emplace_hint_unique(_Rb_tree_const_iterator, _Args... __args)
    -> _Rb_tree_iterator
  {
...
struct map
{
  int &operator[](int __k)
  {
    ...
    if(__i._M_node == end()._M_node)
    {
      ...
      __i =
        _M_t._M_emplace_hint_unique(__i, piecewise_construct, __t1, tuple<>());
    }
    return (*__i).second;
  }
};
