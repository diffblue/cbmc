...
struct H7 : B1
{
  using vt = unsigned long;
  signed char c;
}; // offsetof must skip the alias
union U1
{
  typedef unsigned long td;
  signed char m : 3;
...
{
  __CPROVER_assert(
    sizeof(H1) == 2, "bit-field struct with a static constexpr member");
  __CPROVER_assert(sizeof(H2) == 2, "bit-field struct with a member alias");
  __CPROVER_assert(
    sizeof(H3) == 2, "bit-field struct with a static member and a method");
  ...
  __CPROVER_assert(
    sizeof(H6) == 2 && alignof(H6) == 1,
    "a static member's type does not align the class");
  __CPROVER_assert(
    sizeof(H7) == 24 && offsetof(H7, c) == 16,
    "offsetof skips the member alias");
  __CPROVER_assert(
    sizeof(U1) == 1 && alignof(U1) == 1,
    "union: a member typedef does not size it");
  __CPROVER_assert(
    sizeof(U2) == 2 && alignof(U2) == 2,
    "union: alias, static member and method take no storage");
  __CPROVER_assert(
    sizeof(G<uint16_t>) == 16 && alignof(G<uint16_t>) == 16,
    "attributed class template with a member alias");
  __CPROVER_assert(sizeof(lreg_t) == 16, "alias of the instance");
  __CPROVER_assert(
    sizeof(K<uint16_t>) == 16 && alignof(K<uint16_t>) == 16,
    "aligned class template with a member alias");
  __CPROVER_assert(
    sizeof(Regs) == 48 && offsetof(Regs, b) == 16 &&
      offsetof(Regs, flags) == 32,
  ...
  h.c = 200;
  __CPROVER_assert(
    h.r == RS::S && h.c == 200 && h.size() == 2, "bit-field values");
  return 0;
}
