...
class is_convertible_helper
{
  ...
  template <
    class From1,
    class To1,
    class = decltype(test_aux<To1>(declval<From1>()))>
...
template <class It>
using RequireInputIter = enable_if_t<is_convertible<
...
template <class A>
using RequireAllocator = typename A::value_type;

template <class C, class A = alloc<C>>
struct basic_str
{
  int n;
  basic_str() : n(0)
...
int main()
{
  int arr[3] = {1, 2, 3};
  __CPROVER_assert(
    probe() == 2, "const char* converted through str(const char*)");
  // the trait must still be true after the probe
  __CPROVER_assert(
    is_convertible<random_access_iterator_tag, input_iterator_tag>::value,
    "derived-to-base is convertible");
  vec v(arr, arr + 3);
  __CPROVER_assert(
    v.n == 3,
    "range constructor of a later class uses the same trait instance");
  int *p = arr;
  vec w(p, p + 2);
