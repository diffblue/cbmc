...
int main()
{
  ...
  __CPROVER_assert(
    p + q == 3 && x + y == 15,
    "two declarations in one scope; auto&& to a prvalue aggregate");
  // [class.temporary]/6: the temporary bound to e lives as long as e
  auto &&[m, n] = mk();
  const auto &[c, d] = S{3, 4};
  __CPROVER_assert(
    m + n == 11 && c + d == 7, "auto&& / const auto& to prvalues");
  ...
  __CPROVER_assert(
    r + s == 15 && t + u == 7, "pair prvalue by value and by auto&&");
  int z = 9;
  auto &&[rz, one] = std::pair<int &, int>(z, 1);
  rz = 10;
  __CPROVER_assert(
    z == 10 && one == 1, "binding to a reference member writes through");
  std::pair<int &, int> pr(z, 2);
  auto &[tz, two] = pr;
  tz = 11;
  __CPROVER_assert(
    z == 11 && two == 2, "auto& to an lvalue pair with a reference member");
  int e1 = 2, e2 = 3;
  __CPROVER_assert(
    sum_pair(e1, e2) == 5 && sum_pair(1, 4) == 5,
    "pair<Args&...>(args...) binding");
  std::map<int, W> mp;
  auto res = mp.emplace(1, W{5});
  __CPROVER_assert(
    res.second && mp.at(1).v == 5, "std::map::emplace with two arguments");
  return 0;
}
