...
int f(S) { return 2; }
int g(S &) { return 1; }
int g(const S &) { return 2; }
int h(S &) { return 1; }
int h(S &&) { return 2; }
template <class C> auto t(C &c) -> decltype(c.v, 1) { return 1; }
int t(A) { return 2; }
template <class C> int u(C &c) { return 1; }
int u(A) { return 2; }
struct R { template <class C> auto z(C &c) -> decltype(c.v, 1) { return 1; } int z(A) { return 2; } };
int main()
{
  ...
  __CPROVER_assert(t(A{1}) == 2, "aggregate prvalue: template C& not viable (trailing decltype)");
  __CPROVER_assert(u(A{1}) == 2, "aggregate prvalue: template C& not viable");
  R r;
  __CPROVER_assert(r.z(A{1}) == 2, "member template C& not viable for a prvalue");
  S s(3, 4);
