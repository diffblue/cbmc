...
static inline void with_hard(hard &h, fn<void(hard &hardness)> handler)
{
  handler(h);
}
static inline void
with_std(hard &h, std::function<void(hard &hardness)> handler)
{
  handler(h);
}
static inline void
never_called(hard *p, std::function<void(hard &hardness)> handler)
{
  if(p)
  {
