...
template <typename _Result> struct __is_invocable_impl {
  using _Res_t = typename _Result::type;
  static _Res_t _S_get();
  template <typename _Tp, typename = decltype(_Tp(_S_get())),
            bool _Dangle = __reference_converts_from_temporary(_Tp, _Res_t)>
  static __bool_constant<_Dangle> _S_test(int);
  using type = decltype(_S_test<int>(1));
};
template <typename> struct __call_is_nothrow;
template <typename _Fn, typename... _Args>
...
template <typename _Key, typename, typename _Hash,
