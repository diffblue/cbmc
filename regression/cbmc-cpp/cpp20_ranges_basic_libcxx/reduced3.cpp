...
template <class _Fn>
concept invocable = requires(_Fn __fn) { __fn; };
template <class>
concept __primary_template = requires { typename __void_t<>; };
...
using iter_difference_t =
    conditional_t<__primary_template<iterator_traits<_Ip>>, int,
...
template <class _In>
concept __indirectly_readable_impl = requires(_In __i) { __i; };
template <class _Sp>
concept sized_sentinel_for = requires(_Sp __s) { __s; };
namespace ranges {
template <class _Tp>
concept __member_begin = requires(_Tp __t) { __t; };
...
struct __perfect_forward_impl<_Op, index_sequence<_Idx...>, _BoundArgs...> {
  ...
  template <class _Args> __perfect_forward_impl(_Args);
  template <class... _Args>
...
namespace {
template <class _Yp> void __is_derived_from_view_interface(_Yp);
template <class _Tp>
constexpr bool enable_view =
    requires { __is_derived_from_view_interface<_Tp>(nullptr); };
template <class _Tp>
...
;
template <class _Fn> struct __pipeable : _Fn {};
template <class>
...
template <range _Range> struct ref_view {
  template <class _Tp> ref_view(_Tp);
  _Range begin();
};
...
struct __fn {
  template <class _Tp> auto operator()(_Tp &&__t) { return ref_view{__t}; }
};
namespace {
auto all = __fn{};
}
template <viewable_range _Range> using all_t = decltype(all(declval<_Range>()));
} // namespace ranges::views
...
struct __bind_back_t
    : __perfect_forward<__bind_back_op<tuple_size_v<tuple<int>>>, _Fn,
                        tuple<int>> {};
tuple<> __trans_tmp_1;
template <class _Fn>
...
template <view _View> struct take_view {
  _View __base_;
  _View take_view;
  auto begin() { return ranges::begin(__base_); }
  auto end() { return ranges::begin(__base_); }
};
...
struct Trans_NS___take___fn {
  ...
  template <class _Np> constexpr auto operator()(_Np __n) {
