...
struct __perfect_forward_impl<_Op, index_sequence<_Idx...>, _BoundArgs...>
{
  ...
  __perfect_forward_impl(_Args... __bound_args) : __bound_args_(__bound_args...)
  {
  }
  template <class... _Args>
...
using all_t = decltype(declval<_Range>());
}
...
template <class _Fn, class... _Args>
auto __bind_back(_Fn __f, _Args...)
{
...
{
  counted_iterator(_Iter __iter, iter_difference_t<_Iter>) : __current_(__iter)
  {
  }
  _Iter base()
  {
...
struct take_view
{
  ...
  auto begin()
  {
    return counted_iterator(__base_, __count_);
  }
  auto end()
  {
