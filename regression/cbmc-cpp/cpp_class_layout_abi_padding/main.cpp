...
int main()
{
  ...
  __CPROVER_assert(
    sizeof(U) == 8 && alignof(U) == 4, "union padded to its alignment");
  __CPROVER_assert(sizeof(C) == 16 && __builtin_offsetof(C, s) == 12, "C");
  __CPROVER_assert(sizeof(L) == 1, "L");
  __CPROVER_assert(
    sizeof(BF) == 4 && __builtin_offsetof(BF, d) == 2,
    "bit-field shares its unit, d follows in the next byte");
  __CPROVER_assert(
    sizeof(AL) == 16 && __builtin_offsetof(AL, d) == 12, "alignas");
  ...
  BF bf{1, 2, 3};
  __CPROVER_assert(
    bf.c == 1 && bf.b == 2 && bf.d == 3, "aggregate init with bit-field");
  A arr[2] = {{1, 1}, {2, 2}};
  __CPROVER_assert(
    arr[1].i == 2 && sizeof(arr) == 16, "array of padded structs");
  return 0;
}
