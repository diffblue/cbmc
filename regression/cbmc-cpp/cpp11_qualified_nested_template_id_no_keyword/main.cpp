...
//
// KNOWNBUG (parser): CBMC's parser cannot parse a qualified-id whose
// nested-name-specifier is a class-template-id and whose next component is
// itself a member template-id, when the disambiguating `template` keyword is
// absent:
//
//   C<int>::al<char>::value        // parse error before '> :: value'
//   C<int>::template al<char>::value   // <-- parses (keyword present)
//
// The bug is independent of what the inner member is: it fires for a member
// alias template (`using al = ...`) and for a nested class template
// (`struct N { ... };`) alike, and for a single inner argument (no parameter
// pack needed).  It requires the OUTER name to be a class-template-id
...
// well-formed C++ that CBMC rejects.
//
// Flip to CORE once the parser accepts a member template-id after a
// non-dependent class-template-id without the `template` keyword.
