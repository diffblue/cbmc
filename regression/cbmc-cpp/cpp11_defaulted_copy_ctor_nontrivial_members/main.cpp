...
int main()
{
  ...
  __CPROVER_assert(q.first == "abc" && q.second == 1, "copy of pair<const string, int> (pair's defaulted copy constructor)");
  K k(1, one);
  K k2(k);
  __CPROVER_assert(k2.a == 1 && k2.f() == 1, "defaulted copy constructor with a std::function member");
  K k3(std::move(k2));
  __CPROVER_assert(k3.a == 1 && k3.f() == 1, "defaulted move constructor with a std::function member");
