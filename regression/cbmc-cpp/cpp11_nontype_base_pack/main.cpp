...
{
  using type =
    __make_tuple_types_flat<tuple<>, typename __make_tuple_indices<_Sp>::type>::
...
struct __tuple_leaf
{
  int __value_;
  template <class _Tp>
  __tuple_leaf(_Tp __t) : __value_(__t)
  {
  }
  int get()
  {
...
struct __tuple_impl<__tuple_indices<_Indx...>, _Tp...> : __tuple_leaf<_Indx>...
{
...
int main()
{
  __CPROVER_assert(get<0>(main_t), "first");
}
