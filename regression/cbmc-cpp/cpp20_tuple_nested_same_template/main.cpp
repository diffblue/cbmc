// Same-template NESTING on the operand side: tuple<tuple<int>>
// constructed from a CONVERTING int -- the inner tuple<int>(int)
// element construction runs while the outer tuple<tuple<int>>
// instantiation is mid-flight, and the two instances' same-spelled
// packs (_Tp/_Tf/_Up of the SAME templates) collide in the flat
// template_map when the OUTER mem-init operands are computed (the
// deduction-side collisions were fixed in round 45 via
// deduction_parameters; the operand-side suffix match remains -- the
// documented V1 structural violation, [basic.scope.temp]/2: parameter
// identity is scope + name, not spelling).  The inner constructor's
// body is silently dropped ("no body for callee
// std::tuple<int>::tuple(this,int)") and the assertion fails.
...
int main()
{
  std::tuple<std::tuple<int>> t(3);
  __CPROVER_assert(std::get<0>(std::get<0>(t)) == 3, "converting elem");
  return 0;
}
