...
#include <type_traits>
struct typet { int t; };
struct exprt { int kind; };
struct symbol_exprt : exprt { int id_; const int &identifier() const { return id_; } void identifier(int v) { id_ = v; } };
class ssa_exprt;
struct namespacet {};
enum levelt { L0 = 0, L1 = 1, L2 = 3 };
...
public:
  ...
  const underlyingt &get() const { return static_cast<const underlyingt &>(*this); }
private:
  // a friend declaration naming ANOTHER specialization of this template
  friend renamedt<ssa_exprt, L0> symex_level0(ssa_exprt, const namespacet &, unsigned);
...
class ssa_exprt : public symbol_exprt { public: int level0; };
...
renamedt<ssa_exprt, L0> symex_level0(ssa_exprt s, const namespacet &, unsigned) { return renamedt<ssa_exprt, L0>(s); }
void symex_level1t::insert(const renamedt<ssa_exprt, L0> &ssa, int index) { last = ssa.get().identifier() + index; }
int main()
{
  ssa_exprt s; s.identifier(7); s.level0 = 1; s.kind = 2;
  namespacet ns;
  symex_level1t l; l.insert(symex_level0(s, ns, 0), 1);
  __CPROVER_assert(l.last == 8, "specialization re-elaborated from another specialization's friend declaration");
  return 0;
}
