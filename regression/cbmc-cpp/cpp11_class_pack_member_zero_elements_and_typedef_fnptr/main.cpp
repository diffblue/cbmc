...
public:
  template <class F>
  static R invoke(const any_data &d, A &&...args) { return (*static_cast<F *>(d.p))(std::forward<A>(args)...); }
  template <class F>
  explicit fn(F &f) : functor{&f}, invoker(&invoke<F>) {}
  R operator()(A... args) const { return invoker(functor, std::forward<A>(args)...); }
};
