...
template <class T>
struct __attribute__((packed, aligned(16))) G3
{
  T lo;
  T hi;
};
template <class T>
struct alignas(16) G4
{
  T lo;
  T hi;
};
...
int main()
{
  ...
  __CPROVER_assert(
    sizeof(G3<uint16_t>) == 16, "G3 attribute before body: 16 bytes");
  ...
  __CPROVER_assert(
    sizeof(W) == 32, "W containing an aligned template instance: 32 bytes");
  __CPROVER_assert(
    sizeof(G8<uint16_t>) == 16, "G8 __packed__ __aligned__: 16 bytes");
  __CPROVER_assert(
    sizeof(g1u32_t) == 16 && sizeof(lreg_t) == 16,
    "typedef/alias of the instance");
  G1<uint16_t> g{1, 2};
