...
int main()
{
  int arr[3] = {1, 2, 3};
  for(auto &x : arr) x = 5;
  __CPROVER_assert(arr[0] == 5 && arr[2] == 5, "A: auto& over array writes through");
  S sarr[2] = {{1}, {2}};
  for(auto &s : sarr) s.v = 9;
  ...
  ls[0].push_back(1);
  for(const auto &l : ls)
    __CPROVER_assert(!l.empty() && l.front() == 1, "E: const auto& to a list element (no bitwise copy)");
