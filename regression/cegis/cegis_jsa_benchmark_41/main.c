...
int main(void)
{
  ...
  const __CPROVER_jsa_list_id_t __CPROVER_jsa_list_array=0;
  __CPROVER_jsa_assume_valid_list(&heap, __CPROVER_jsa_list_array);
  const __CPROVER_jsa_list_id_t __CPROVER_jsa_list_result=1;
  __CPROVER_jsa_assume_new_list(&heap, __CPROVER_jsa_list_result);
  for (const __CPROVER_jsa_iterator_id_t __CPROVER_jsa_iterator_it=__CPROVER_jsa_iterator(&heap, __CPROVER_jsa_list_array);
       __CPROVER_jsa_hasNext(&heap, __CPROVER_jsa_iterator_it);)
  {
    ...
    if (__CPROVER_jsa_mod(tmp, 3) == 0)
    {
      __CPROVER_jsa_add(&heap, __CPROVER_jsa_list_result, tmp);
    }
  }
