...
int main(void)
{
  ...
  const __CPROVER_jsa_list_id_t __CPROVER_jsa_list_mPrimes;
  __CPROVER_jsa_assume_valid_list(&heap, __CPROVER_jsa_list_mPrimes);
  const __CPROVER_jsa_word_t x;
  const __CPROVER_jsa_iterator_id_t __CPROVER_jsa_iterator_it=__CPROVER_jsa_iterator(&heap, __CPROVER_jsa_list_mPrimes);
  for (;__CPROVER_jsa_hasNext(&heap, __CPROVER_jsa_iterator_it);)
  {
    ...
    if(__CPROVER_jsa_mult(num, num) > x)
    {
