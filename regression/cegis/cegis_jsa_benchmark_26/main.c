...
int main(void)
{
  ...
  const __CPROVER_jsa_data_t zero=0;
  const __CPROVER_jsa_data_t num;
  ...
  {
    const __CPROVER_jsa_data_t number=__CPROVER_jsa_next(&heap, __CPROVER_jsa_iterator_it);
    if (__CPROVER_jsa_mod(number, num) == zero)
    {
