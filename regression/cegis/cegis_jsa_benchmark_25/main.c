...
int main(void)
{
  ...
  const __CPROVER_jsa_list_id_t __CPROVER_jsa_list_Integers=0;
  __CPROVER_jsa_assume_valid_list(&heap, __CPROVER_jsa_list_Integers);
  const __CPROVER_jsa_list_id_t __CPROVER_jsa_list_ret=1;
  __CPROVER_jsa_assume_new_list(&heap, __CPROVER_jsa_list_ret);
  for (const __CPROVER_jsa_iterator_id_t __CPROVER_jsa_iterator_it=__CPROVER_jsa_iterator(&heap, __CPROVER_jsa_list_Integers);
       __CPROVER_jsa_hasNext(&heap, __CPROVER_jsa_iterator_it);)
  {
    const __CPROVER_jsa_word_t tmp=__CPROVER_jsa_next(&heap, __CPROVER_jsa_iterator_it);
    __CPROVER_jsa_add(&heap, __CPROVER_jsa_list_ret, tmp);
  }
