...
public:
  ...
  void add_xor (const std::vector<int> &lits, bool rhs);

  struct DerivedClause { std::vector<int> lits; };
  std::vector<DerivedClause> gauss_eliminate();
  ...
  //
  void phase (int lit);
  void decide_first (int lit);  // Add variable to priority decision queue
  void bump (int lit);  // Bump variable activity (for external hinting)
