...
cutpoint_edge_ptrt
cutpoint_grapht::get_edge(const cutpointt &s, const cutpointt &d)
{
  for(auto it = s.succ_begin(), end = s.succ_end(); it != end; ++it)
  {
    cutpoint_edge_ptrt edg = *it;
    if(&(edg->target()) == &d)
      return edg;
  }
  ...
  forall_goto_program_instructions(it, goto_function.body)
  {
    if(is_cutpoint(*it))
    {
      ...
      for(std::size_t r = 0; r < reach.size(); r++)
      {
        if(reach[r] == true)
        {
          cutpoint_edge_ptrt edg = create_edge(cp, *m_cps[r]);
          edg->push_back(&(*it));
        }
      }
    }
    else
    {
      ...
      for(std::size_t i = 0; i < breach.size(); i++)
      {
        ...
        for(std::size_t j = 0; j < freach.size(); j++)
        {
          if(freach[j] == false)
            continue;
          cutpoint_edge_ptrt edge = get_edge(*m_cps[i], *m_cps[j]);
          edge->push_back(&(*it));
        }
      }
    }
...
{
  for(auto it = goto_function.body.instructions.rbegin();
      it != goto_function.body.instructions.rend();)
  {
    auto itf = (++it).base();
