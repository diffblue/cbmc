...
Module: CHC Weak Topological Ordering
...
/// \file
/// Weak topological ordering (WTO) for CHC dependency graphs
...
public:
  explicit wto_singletont(const symbol_exprt *e) : m_singleton(e)
  {
  }

  const symbol_exprt *get() const
  {
    return m_singleton;
  }

  void accept(wto_element_visitort *v) override
  {
    v->visit(*this);
  }
};
...
{
public:
  typedef std::shared_ptr<wto_elementt> wto_element_ptrt;
  typedef std::deque<wto_element_ptrt> wto_componentst;

private:
  const symbol_exprt *m_head;
  wto_componentst m_components;

public:
  wto_componentt(const symbol_exprt *head, wto_componentst components)
    : m_head(head), m_components(components)
  {
  }

  typename wto_componentst::iterator begin()
  {
    return m_components.begin();
  }

  typename wto_componentst::iterator end()
  {
    return m_components.end();
  }

  typename wto_componentst::const_iterator begin() const
  {
    return m_components.begin();
  }

  typename wto_componentst::const_iterator end() const
  {
    return m_components.end();
  }

  const symbol_exprt *head() const
  {
    return m_head;
  }
...
private:
  ...
  public:
    ...
    explicit inf_numt(unsigned n) : m_n(n)
    {
    }
    ...
    {
      return m_n == std::nullopt;
    }
    bool is_finite() const
    {
      return !is_plus_infinite();
    }
  ...
  typedef std::shared_ptr<wto_elementt> wto_element_ptrt;
  typedef std::deque<wto_element_ptrt> partitiont;
  ...
  std::deque<wto_element_ptrt> m_partition;
  nested_componentst m_nested_comp;
...
private:
  ...
  {
    std::deque<wto_element_ptrt> partition;
  ...
  class nested_components_visitort : public wto_element_visitort
  {
    std::vector<wto_componentt> m_nested_components;
    nested_componentst &m_nested_components_table;

  public:
    explicit nested_components_visitort(nested_componentst &t)
      : wto_element_visitort(), m_nested_components_table(t)
    {
    }

    void visit(const wto_singletont &s) override
    {
      m_nested_components_table[s.get()->hash()] = m_nested_components;
    }

    void visit(const wto_componentt &c) override
    {
  ...
  {
    nested_components_visitort vis(m_nested_comp);
...
public:
  explicit chc_wtot(chc_grapht &g) : m_g(g), m_cur_dfn_num(0)
  {
  }
  ...
  partitiont::iterator begin()
  {
    return m_partition.begin();
  }
  partitiont::iterator end()
  {
    return m_partition.end();
  }
  partitiont::const_iterator begin() const
  {
    return m_partition.begin();
  }
  partitiont::const_iterator end() const
  {
    return m_partition.end();
  }
  ...
  std::vector<wto_componentt>::const_iterator
  nested_comp_begin(symbol_exprt *p) const
  {
  ...
  std::vector<wto_componentt>::const_iterator
  nested_comp_end(symbol_exprt *p) const
  {
    auto it = m_nested_comp.find(p->hash());
    INVARIANT(it != m_nested_comp.end(), "No nested component");
...
#endif // CPROVER_CPROVER_CHC_WTO_H
