...
/// \file
/// CHC large step encoding via resolution

#ifndef CPROVER_CPROVER_CHC_LARGE_STEP_H
#define CPROVER_CPROVER_CHC_LARGE_STEP_H
...
public:
  explicit resolution_visitort(chc_dbt &db) : m_db(db), m_verbose(false)
  {
  }

  chc_dbt &giveme_new_db()
  {
    return m_new_db;
  }
...
private:
    ...
    for(auto idx : m_db.def(*symb))
    {
      ...
      if(use.size() > 1)
      {
        throw incorrect_goto_program_exceptiont(
          "Non-linear CHCs not supported yet");
      }
      ...
      for(auto &p : use)
      {
        ...
        {
          for(auto cls_it = it->second.begin(); cls_it != it->second.end();
              cls_it++)
          {
  ...
  forall_exprt resolve_clauses(const horn_clauset &c1, const horn_clauset &c2)
  {
    ...
    std::set<symbol_exprt> all_vars(
      c1.get_chc().variables().begin(), c1.get_chc().variables().end());
    all_vars.insert(
      c2.get_chc().variables().begin(), c2.get_chc().variables().end());
    ...
    exprt transformed_body = (can_cast_expr<and_exprt>(body2))
                               ? to_and_expr(body2).op1()
                               : true_exprt();
    exprt transformed_head = head2;

    INVARIANT(
      head1_pred->arguments().size() == 1,
      "Only one argument to predicate is assumes");
    const exprt &head_arg = head1_pred->arguments().at(0);
    const symbol_exprt &body_arg = to_symbol_expr(body2_pred.arguments().at(0));

    if(
      (head_arg.id() != ID_symbol) ||
    ...
    forall_exprt f(
      std::vector<symbol_exprt>(all_vars.begin(), all_vars.end()),
      implies_exprt(std::move(transformed_body), std::move(transformed_head)));
    return f;
  }
};

#endif // CPROVER_CPROVER_CHC_LARGE_STEP_H
