...
#include "inking_class.h"

std::string inkingt::expr_to_string(
  const namespacet &ns,
...
std::string inkingt::type_to_string(
  const namespacet &ns,
  const irep_idt &identifier,
...
std::string inkingt::type_to_string_verbose(
  const namespacet &ns,
...
void inkingt::detailed_conflict_report_rec(
...
irep_idt inkingt::rename(const irep_idt id)
{
  unsigned cnt=0;
  ...
  while(true)
  {
    ...
    if(src_symbol_table.symbols.find(new_identifier)!=
      ...
      continue; // used by some earlier inking call already

    return new_identifier;
  }
}

bool inkingt::needs_renaming_non_type(
  const symbolt &old_symbol,
  const symbolt &new_symbol)
{
...
void inkingt::duplicate_code_symbol(
  symbolt &old_symbol,
  symbolt &new_symbol)
{
  // Both are functions.
...
bool inkingt::adjust_object_type_rec(
  const typet &t1,
  const typet &t2,
...
bool inkingt::adjust_object_type(
  const symbolt &old_symbol,
  const symbolt &new_symbol,
  bool &set_to_new)
{
...
void inkingt::duplicate_object_symbol(
  symbolt &old_symbol,
  symbolt &new_symbol)
{
  // both are variables
...
void inkingt::duplicate_non_type_symbol(
  symbolt &old_symbol,
  symbolt &new_symbol)
{
...
void inkingt::duplicate_type_symbol(
  symbolt &old_symbol,
  const symbolt &new_symbol)
{
...
bool inkingt::needs_renaming_type(
  const symbolt &old_symbol,
  const symbolt &new_symbol)
{
  assert(new_symbol.is_type);
...
void inkingt::rename_symbols(const id_sett &needs_to_be_renamed)
{
