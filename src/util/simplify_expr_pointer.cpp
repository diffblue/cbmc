...
bool simplify_exprt::simplify_address_of(exprt &expr)
{
  ...
  {
    // simplify &*p to p
    PRECONDITION(object.operands().size()==1);
...
bool simplify_exprt::simplify_inequality_address_of(exprt &expr)
{
  ...
  DATA_INVARIANT(tmp0.id()==ID_address_of, "id must be ID_address_of");
  DATA_INVARIANT(tmp1.id()==ID_address_of, "id must be ID_address_of");
