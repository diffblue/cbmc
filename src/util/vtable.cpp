...
const char vtable_namest::VT_PTR[] = "@vtable_pointer";

std::string vtable_namest::get_type(const std::string &class_name) {
  return VT_NS + class_name;
}

std::string vtable_namest::get_table(const std::string &class_name) {
  return get_type(class_name) + VT_SEP + class_name;
}

std::string vtable_namest::get_table(const typet &class_type) {
...
std::string vtable_namest::get_table(const exprt &this_obj) {
  return get_table(this_obj.type());
}


std::string vtable_namest::get_table_base(
    const std::string &class_base_name) {
  return get_type_base(class_base_name) + VT_SEP + class_base_name;
}

std::string vtable_namest::get_type(const typet &class_type) {
  const irep_idt &type_id(class_type.id());
...
std::string vtable_namest::get_type(const exprt &this_obj) {
  return get_type(this_obj.type());
}

std::string vtable_namest::get_type_base(
    const std::string &class_base_name) {
  return VT_NS + class_base_name;
}

std::string vtable_namest::get_pointer(const std::string &class_name) {
