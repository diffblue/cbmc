    ...
    operands[1].type().id() == ID_pointer)
  {
    ...
    if(ptr_op0 == ptr_op1 && address_of)
    {
      ...
      if(
        ...
        element_size->is_constant() && *element_size != 0 &&
...
simplify_exprt::simplify_bitwise(const multi_ary_exprt &expr)
{
  ...
  if(new_expr.id() == ID_bitor || new_expr.id() == ID_bitxor)
  {
    ...
    {
      if(*it == 0 && new_expr.operands().size() > 1)
      {
...
simplify_exprt::simplify_extractbits(const extractbits_exprt &expr)
{
  ...
  const auto end = numeric_cast<bitst>(expr.index());
  ...
  const auto start = std::optional(*end + *result_width - bitst{1});
  ...
  else if(auto eb_src = expr_try_dynamic_cast<extractbits_exprt>(expr.src()))
  {
    if(eb_src->index().is_constant())
    {
      ...
      const bitst src_index =
        numeric_cast_v<bitst>(to_constant_expr(eb_src->index()));
...
static bool eliminate_common_addends(exprt &op0, exprt &op1)
{
  ...
  {
    if(op0 != 0 && op0.type().id() != ID_complex)
    {
      // elimination!
  ...
  const binary_relation_exprt &expr)
{
    ...
    if(op1_constant.is_null_pointer())
    {
      ...
      else if(expr.op0().id() == ID_plus)
      {
        ...
        if(ptr.is_constant() && to_constant_expr(ptr).is_null_pointer())
          return make_boolean_expr(offset == 0);
    ...
    to_typecast_expr(expr.op0()).op().is_boolean())
  {
    ...
    if(expr.op1() == 0 && expr.id() == ID_equal)
    {
      return changed(simplify_not(not_exprt(lhs_typecast_op)));
    }

    // we re-write (TYPE)boolean != 0 -> boolean
    if(expr.op1() == 0 && expr.id() == ID_notequal)
    {
