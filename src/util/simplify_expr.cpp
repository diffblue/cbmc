...
bool simplify_exprt::simplify_node(exprt &expr)
{
  ...
  {
    auto simplify_result = simplify_floatbv_typecast(to_floatbv_typecast_expr(expr));
    bool unchanged = simplify_result.was_changed == simplify_exprt::was_changedtt::Unchanged;
    if(!unchanged)
    {
      expr = simplify_result.new_expr;
    }
