  ...
  const exprt &src_ssa)      // renamed identifiers
{
  ...
  if(id==ID_index)
  {
    ...
    if(index_value.is_not_nil())
    {
      ...
      tmp.array()=
        build_full_lhs_rec(decision_procedure, ns,
  ...
  else if(id==ID_if)
  {
    ...
    tmp2.false_case()=build_full_lhs_rec(decision_procedure, ns,
      tmp2.false_case(), to_if_expr(src_ssa).false_case());

    tmp2.true_case()=build_full_lhs_rec(decision_procedure, ns,
  ...
  else if(id==ID_typecast)
  {
    ...
    tmp.op()=build_full_lhs_rec(decision_procedure, ns,
    ...
    for(const auto ssa_step_it : time_and_ssa_steps.second)
    {
      ...
      if(SSA_step.original_full_lhs.is_not_nil())
      {
        goto_trace_step.full_lhs = build_full_lhs_rec(
          decision_procedure, ns, SSA_step.original_full_lhs, SSA_step.ssa_full_lhs);
        replace_nondet_in_type(goto_trace_step.full_lhs, decision_procedure);
      }

      if(SSA_step.ssa_full_lhs.is_not_nil())
      {
        goto_trace_step.full_lhs_value = decision_procedure.get(SSA_step.ssa_full_lhs);
  ...
  goto_tracet &goto_trace)
{
  const auto is_last_step_to_keep = [last_step_to_keep](
    symex_target_equationt::SSA_stepst::const_iterator it, const decision_proceduret &) {
...
static bool is_failed_assertion_step(
  ...
  const decision_proceduret &decision_procedure)
{
  return step->is_assert() && decision_procedure.get(step->cond_handle).is_false();
}
...
void build_goto_trace(
  ...
  goto_tracet &goto_trace)
{
  build_goto_trace(target, is_failed_assertion_step, decision_procedure, ns, goto_trace);
}
