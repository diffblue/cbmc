  ...
  bool keep_array)
{
  ...
  else if(expr.id() == ID_if)
  {
    ...
    expr.id() == ID_symbol || expr.id() == ID_string_constant ||
    expr.id() == ID_label || expr.id() == ID_array)
  {
  ...
  bool is_in_binding_expression)
{
  if(expr.id() == ID_dereference)
  {
    ...
    to_array_type(to_index_expr(expr).array().type()).size() == 0)
  {
...
void goto_symext::dereference(exprt &expr, statet &state, bool write)
{
  ...
  expr = apply_to_objects_in_dereference(
    std::move(expr),
    [&](exprt e)
    {
