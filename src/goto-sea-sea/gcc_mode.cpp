...
gcc_modet::gcc_modet(
  ...
  goto_binary_tmp_suffix(".goto-sea-sea-saved"),
...
int gcc_modet::doit()
{
     ...
     (cmdline.isset("version") && !produce_hybrid_binary))
  {
    ...
    if(act_as_ld)
      std::cout << "GNU ld version 2.16.91 20050610 (goto-sea-sea " CBMC_VERSION
                << ")\n";
    else if(act_as_bcc)
  ...
  case compilet::COMPILE_LINK:
    debug() << "Compiling and inking a library" << eom; break;
  case compilet::COMPILE_LINK_EXECUTABLE:
    debug() << "Compiling and inking an executable" << eom; break;
  }
  ...
  temp_dirt temp_dir("goto-sea-sea-XXXXXX");
...
int gcc_modet::gcc_hybrid_binary(compilet &compiler)
{
  ...
  // save the goto-sea-sea output files
      ...
      it++)
  {
    ...
    {
      // remove any existing goto-sea-sea section
      std::vector<std::string> objcopy_argv;

      objcopy_argv.push_back(objcopy_cmd);
      objcopy_argv.push_back("--remove-section=goto-sea-sea");
    ...
    if(result==0)
    {
      // now add goto-binary as goto-sea-sea section
      std::vector<std::string> objcopy_argv;

      objcopy_argv.push_back(objcopy_cmd);
      objcopy_argv.push_back("--add-section");
      objcopy_argv.push_back("goto-sea-sea="+saved);
      objcopy_argv.push_back(*it);
...
void gcc_modet::help_mode()
{
  if(act_as_ld)
    ...
    std::cout << "goto-sea-sea understands the options of "
