...
{
public:
  json_objectt operator()(const exprt &expr, const namespacet &ns) override
  {
    return json_exprt::operator()(expr, ns);
  }

  json_objectt operator()(const typet &type, const namespacet &ns) override
  {
...
protected:
  ...
  std::string
  from_constant(const namespacet &ns, const constant_exprt &) override;
