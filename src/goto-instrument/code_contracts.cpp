  ...
  const loopt &loop)
{
  ...
  // section of setup.
  if(loop_head->is_goto())
    loop_head->make_assumption(boolean_negate(loop_head->guard));
  ...
  // Now havoc at the loop end. Use insert_before_swap to preserve jumps to loop
  // end.
  ...
  goto_programt &dest)
{
  ...
  if(goto_function.type.return_type() != empty_typet())
  {
    ...
    symbol_exprt r =
      new_tmp_symbol(goto_function.type.return_type(), d->source_location)
        .symbol_expr();
    d->code = code_declt(r);
    ...
    symbol_exprt old(
      "__CPROVER_return_value", goto_function.type.return_type());
  ...
  for(const auto &p_it : goto_function.type.parameters())
  {
    ...
    if(!p_it.get_identifier().empty())
    {
      symbol_exprt old(p_it.get_identifier(), p_it.type());
      replace.insert(old, p);
    }
  }
