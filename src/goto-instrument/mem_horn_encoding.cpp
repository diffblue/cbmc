...
symbol_exprt unprime(symbol_exprt s)
{
  ...
  if (unpr == exprt()) return s;
  else return to_symbol_expr(unpr);
}
...
void rewrite_derefs(exprt &expr)
{
  if (expr.id() == ID_if)
  {
    ...
    if (cond.id() == ID_equal && cond.operands()[0].id() == ID_address_of
            ...
            exprt dst_inv, bool can_skip)
{
  ...
  if (can_skip &&
      body.size() == 1 &&
      dst_inv.id() == ID_function_application &&
  ...
  for (auto it = statements.crbegin(); it != statements.crend(); ++it)
  {
    ...
    else if (stmt == ID_assign || stmt == ID_decl)
    {
      ...
      for (auto & s : cur_dep_symbols)
      {
        ...
        if (use_mem) rewrite_derefs(wp_sym);
        next_vars[s] = wp_sym;
