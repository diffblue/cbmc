...
bool event_grapht::critical_cyclet::is_not_weak_uniproc() const
{
  ...
  for(;
    it!=end() && (
    (egraph[*it].variable==var &&
      (it==begin() || egraph[*it].operation!=abstract_eventt::Read
  ...
  bool local_write)
{
  ...
  {
    std::set<data_classt>::iterator next = class_it;
    ++next;
...
void data_dpt::dp_merge()
{
      ...
      ++class_it)
  {
    ...
    ++class_plus_one;
    //std::set<data_classt>::iterator next_class = class_it;
    //++next_class;
    ...
    for(std::set<data_classt>::iterator class_it2=class_plus_one;
        ...
        ++class_it2)
    {
      //std::set<data_classt>::iterator next_class2 = class_it2;
      //++next_class2;
      bool next = false;
...
{
#ifdef DEBUG
  for(unsigned i=0; i++<80; std::cout << "-");
  std::cout << std::endl;
  ...
  if(!this_vertex.local)
  {
    ...
    if(lwfence_met && this_vertex.operation!=abstract_eventt::Read)
      {no_comm=true;get_com_only=false;}//return false;
...
void test2()
{
  ...
  for(std::set<event_grapht::critical_cyclet>::iterator it=set.begin(); 
    it!=set.end(); it++)
    if(it->is_not_uniproc(Power) && it->is_not_thin_air())
