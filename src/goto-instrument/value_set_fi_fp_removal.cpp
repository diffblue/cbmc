...
std::vector<exprt> flatten_structs(const exprt &src, const namespacet &ns)
{
...
public:
  ...
  void merge_ids(const irep_idt &id1, optionalt<irep_idt> &id2);
...
aliasest::id_sett aliasest::get_addresses(const exprt &src)
{
  ...
  {
#ifdef USE_STD_STRING
    unsigned root = uuf.find(dstringt(id).get_no());
    ...
    {
#ifdef USE_STD_STRING
      irep_idt alias_id = as_string(dstringt::make_from_table_index(alias));
#else
...
void aliasest::merge_ids(const irep_idt &id1, optionalt<irep_idt> &id2)
{
  if(id2.has_value())
  {
#ifdef USE_STD_STRING
    unsigned no1 = dstringt(id1).get_no();
    unsigned no2 = dstringt(id2.value()).get_no();
#else
  ...
  optionalt<irep_idt> &id)
{
  ...
  else if(src.id() == ID_member)
  {
    ...
    merge_ids_rec(
      m.struct_op(), "." + id2string(m.get_component_name()) + suffix, id);
  }
  ...
  id_sett &dest)
{
  ...
  else if(src.id() == ID_member)
  {
    const auto &m = to_member_expr(src);
    get_ids_rec(
      m.struct_op(), "." + id2string(m.get_component_name()) + suffix, dest);
  }
  else if(src.id() == ID_index)
  {
    const auto &i = to_index_expr(src);
  ...
  const code_function_callt &function_call)
{
  ...
  const auto called_f_it = goto_functions.function_map.find(function_id);
  ...
  if(lhs.is_not_nil())
  {
    const irep_idt return_value_id = id2string(function_id) + "#return_value";
        ...
        else if(i.is_function_call())
        {
          ...
          if(function.id() == ID_dereference)
          {
            ...
            for(const auto &address : addresses)
            {
              ...
              if(deref.type().id() == ID_code && deref.id() == ID_symbol)
              {
                const irep_idt &function_id =
                  to_symbol_expr(deref).get_identifier();
                do_function_call(
                  goto_model.goto_functions, function_id, function_call);
              }
            }
          }
  ...
  const namespacet &ns)
{
  ...
  for(std::size_t i = 0; i < function_parameters.size(); i++)
  {
    ...
    {
      if(!base_type_eq(
           call_arguments[i].type(), function_parameters[i].type(), ns))
      {
        call_arguments[i] =
  ...
  goto_modelt &goto_model)
{
  ...
  // type already ok?
  if(base_type_eq(function_call.lhs().type(), code_type.return_type(), ns))
    return;
