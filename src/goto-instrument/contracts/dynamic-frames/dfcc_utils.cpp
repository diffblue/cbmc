...
const bool dfcc_utilst::function_symbol_exists(const std::string &function_id)
{
  return symbol_exists(function_id, true, false);
}

const bool
dfcc_utilst::function_symbol_with_body_exists(const std::string &function_id)
{
  ...
  code_typet::parameterst &new_params)
{
  ...
  for(auto &old_param : old_params)
  {
    ...
    INVARIANT(
      goto_model.symbol_table.lookup(new_param_id) == NULL, "name clash");
  ...
  const irep_idt &wrapped_function_id)
{
  ...
  // Check that new name really exists
  INVARIANT(
  ...
  // Check that the slot is empty in function_map
  INVARIANT(
...
void dfcc_utilst::check_loop_freeness(
  const irep_idt &function_id,
  std::string msg,
  int err)
{
  auto &goto_function = goto_model.goto_functions.function_map.at(function_id);
  if(!is_loop_free(goto_function.body, ns, log))
  {
    log.error() << "loop-freeness check failed for function '"
                << id2string(function_id) << "': " << msg << messaget::eom;
    throw err;
  }
}
