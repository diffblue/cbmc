  ...
  const cpp_typecheck_fargst &fargs)
{
  ...
  else if(expr.id()==ID_ptrmember)
  {
    ...
    if(expr.op0().type().id() != ID_pointer)
    {
      ...
      function_call.add_source_location()=expr.location();
...
void cpp_typecheckt::typecheck_expr_explicit_typecast(exprt &expr)
{
  ...
  if(expr.operands().size()==0)
  {
    ...
    new_expr.add_source_location()=expr.location();
    expr=new_expr;
  }
  else if(expr.operands().size()==1)
  {
    ...
    if(expr.type().id()==ID_cpp_name)
    {
      ...
      else
      {
        ...
        f_call.add_source_location()=expr.location();
...
void cpp_typecheckt::typecheck_expr_explicit_constructor_call(exprt &expr)
{
  ...
  else
  {
    ...
    symb.add_source_location()=expr.location();
...
void cpp_typecheckt::typecheck_expr_delete(exprt &expr)
{
  ...
  new_object.add_source_location()=expr.location();
  ...
  const cpp_typecheck_fargst &fargs)
{
       ...
       symbol_expr.op0().is_nil())
    {
      if(symbol_expr.type().get(ID_return_type)!=ID_constructor)
      {
        ...
        else
        {
          ...
          ptrmem.add_source_location()=location;
  ...
  symbol_expr.add_source_location()=location;
  expr=symbol_expr;
  ...
  side_effect_expr_function_callt &expr)
{
  ...
  if(expr.function().id()=="pod_constructor")
  {
    ...
    else if(expr.arguments().size()==1)
    {
      ...
      typecast.type()=pod;
      typecast.add_source_location() = expr.location();
  ...
  if(expr.function().type().id()==ID_pointer)
  {
    ...
    else
    {
      ...
      tmp.add_source_location()=expr.op0().location();
  ...
  else
  {
       ...
       expr.arguments().size()>=1)
    {
         ...
         operand.type()==argument.type().subtype())
      {
        ...
        tmp.add_source_location()=operand.location();
  ...
  side_effect_expr_function_callt &expr)
{
  ...
  new_function.add_source_location()=member_expr.location();
...
void cpp_typecheckt::explicit_typecast_ambiguity(exprt &expr)
{
      ...
      op0_id==ID_dereference))
  {
    ...
    if(resolve_result.id()!=ID_type)
    {
      ...
      new_binary_expr.add_source_location()=expr.op0().location();
