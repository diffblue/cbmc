  ...
  const typet &desired_type)
{
  ...
  if(template_type.id() == ID_cpp_name)
  {
    ...
    if(cpp_name.has_template_args())
    {
      ...
      {
        // Fallback for forward-declared template instantiations
        // (e.g., basic_ostream in libc++-20's __fwd/ headers).
        irep_idt cpp_base = cpp_name.get_base_name();
        if(!cpp_base.empty() && desired_sym->base_name == cpp_base)
        {
          std::string id_str = id2string(desired_id);
          auto angle = id_str.find('<');
          if(angle != std::string::npos)
          {
            std::string suffix = id_str.substr(angle + 1);
            if(!suffix.empty() && suffix.back() == '>')
              suffix.pop_back();
            std::vector<std::string> id_args;
            int depth = 0;
            std::string current;
            for(char ch : suffix)
            {
            ...
            if(!current.empty())
              id_args.push_back(current);
            if(id_args.size() == targs.size())
            {
              bool all_ok = true;
              for(std::size_t i = 0; i < targs.size(); i++)
              {
                typet arg_t;
                const std::string &a = id_args[i];
                if(a.find("tag-") != std::string::npos)
                  arg_t = struct_tag_typet{irep_idt{a}};
                else if(a == "char")
                  arg_t = char_type();
                else if(a == "wchar_t")
                  arg_t = wchar_t_type();
                else if(a == "signed_int")
                  arg_t = signed_int_type();
                else
                {
                  all_ok = false;
                  break;
          ...
          // Base name matched but args couldn't be parsed.
          // Don't poison — another instantiation may match.
          return;
        }
        mark_targs_conflicting();
        return;
