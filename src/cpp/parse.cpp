...
{
  return t == TOK_DYNAMIC_CAST || t == TOK_STATIC_CAST ||
         t == TOK_REINTERPRET_CAST || t == TOK_CONST_CAST;
}
...
bool Parser::rPostfixExpr(exprt &exp)
{
  ...
  // a primary-expression
    ...
    optIntegralTypeOrClassSpec(type) && type.is_not_nil() &&
    (lex.LookAhead(0) == '(' || lex.LookAhead(0) == '{'))
  {
#ifdef DEBUG
  ...
  else
  {
      ...
      (lex.LookAhead(0) == '(' || lex.LookAhead(0) == '{'))
    {
      ...
      side_effect_expr_function_callt fc(
        std::move(type_or_function_name),
        exp2.operands(),
        typet{},
        source_locationt{});
      set_location(fc, tk);
