...
{
  return add_axioms_from_int(args(expr, 1)[0], MAX_INTEGER_LENGTH);
}
...
string_exprt string_constraint_generatort::add_axioms_from_long(
  const function_application_exprt &expr)
{
  return add_axioms_from_int(args(expr, 1)[0], MAX_LONG_LENGTH);
}
  ...
  const exprt &f, bool double_precision)
{
  ...
  const exprt &index24=from_integer(24, get_index_type());
...
Function: string_constraint_generatort::add_axioms_from_char
 ...
 Purpose: add axioms stating that the returned string as length 1 and
