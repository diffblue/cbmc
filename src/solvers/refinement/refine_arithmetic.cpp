...
void bv_refinementt<bv_pointers_baset>::approximationt::add_over_assumption(
  literalt l)
{
...
template <typename bv_pointers_baset>
void bv_refinementt<bv_pointers_baset>::approximationt::add_under_assumption(
  literalt l)
{
  // if it's a constant already, give up
  if(!l.is_constant())
...
template <typename bv_pointers_baset>
bvt bv_refinementt<bv_pointers_baset>::convert_floatbv_op(
  const ieee_float_op_exprt &expr)
{
  if(!config_.refine_arithmetic)
    return bv_pointers_baset::convert_floatbv_op(expr);
...
template <typename bv_pointers_baset>
bvt bv_refinementt<bv_pointers_baset>::convert_mult(const mult_exprt &expr)
{
  if(!config_.refine_arithmetic || expr.type().id()==ID_fixedbv)
    return bv_pointers_baset::convert_mult(expr);
  ...
  {
    // x*0==0 and 0*x==0
    literalt op0_zero = this->bv_utils.is_zero(a.op0_bv);
    literalt op1_zero = this->bv_utils.is_zero(a.op1_bv);
    literalt res_zero = this->bv_utils.is_zero(a.result_bv);
    this->prop.l_set_to_true(
      this->prop.limplies(this->prop.lor(op0_zero, op1_zero), res_zero));

    // x*1==x and 1*x==x
    literalt op0_one = this->bv_utils.is_one(a.op0_bv);
    literalt op1_one = this->bv_utils.is_one(a.op1_bv);
    literalt res_op0 = this->bv_utils.equal(a.op0_bv, a.result_bv);
    literalt res_op1 = this->bv_utils.equal(a.op1_bv, a.result_bv);
    this->prop.l_set_to_true(this->prop.limplies(op0_one, res_op1));
    this->prop.l_set_to_true(this->prop.limplies(op1_one, res_op0));
  }
...
template <typename bv_pointers_baset>
bvt bv_refinementt<bv_pointers_baset>::convert_div(const div_exprt &expr)
{
  if(!config_.refine_arithmetic || expr.type().id()==ID_fixedbv)
    return bv_pointers_baset::convert_div(expr);
...
template <typename bv_pointers_baset>
bvt bv_refinementt<bv_pointers_baset>::convert_mod(const mod_exprt &expr)
{
  if(!config_.refine_arithmetic || expr.type().id()==ID_fixedbv)
    return bv_pointers_baset::convert_mod(expr);
...
template <typename bv_pointers_baset>
void bv_refinementt<bv_pointers_baset>::get_values(approximationt &a)
{
  ...
  {
    a.op0_value = this->get_value(a.op0_bv);
    a.op1_value = this->get_value(a.op1_bv);
  }
  else if(o==3)
  {
    a.op0_value = this->get_value(a.op0_bv);
    a.op1_value = this->get_value(a.op1_bv);
    a.op2_value = this->get_value(a.op2_bv);
  }
  else
    UNREACHABLE;

  a.result_value = this->get_value(a.result_bv);
}

/// inspect if satisfying assignment extends to original formula, otherwise
/// refine overapproximation
template <typename bv_pointers_baset>
void bv_refinementt<bv_pointers_baset>::check_SAT(approximationt &a)
{
  ...
  if(type.id()==ID_floatbv)
  {
    ...
    constant_exprt rounding_mode_expr =
      to_constant_expr(this->get(float_op.rounding_mode()));
    ...
    this->log.debug() << "S1: " << o0 << " " << a.expr.id() << " " << o1
                      << " != " << rr << messaget::eom;
    this->log.debug() << "S2: " << integer2binary(a.op0_value, spec.width())
                      << " " << a.expr.id() << " "
                      << integer2binary(a.op1_value, spec.width())
                      << "!=" << integer2binary(a.result_value, spec.width())
                      << messaget::eom;
    this->log.debug() << "S3: " << integer2binary(a.op0_value, spec.width())
                      << " " << a.expr.id() << " "
                      << integer2binary(a.op1_value, spec.width())
    ...
    {
      bvt r;
      float_utilst float_utils(this->prop);
      float_utils.spec=spec;
      float_utils.rounding_mode_bits.set(rounding_mode);

      literalt op0_equal =
        this->bv_utils.equal(a.op0_bv, float_utils.build_constant(o0));

      literalt op1_equal =
        this->bv_utils.equal(a.op1_bv, float_utils.build_constant(o1));

      literalt result_equal =
        this->bv_utils.equal(a.result_bv, float_utils.build_constant(result));

      literalt op0_and_op1_equal = this->prop.land(op0_equal, op1_equal);

      this->prop.l_set_to_true(
        this->prop.limplies(op0_and_op1_equal, result_equal));
    }
    else
    {
      // give up
          ...
          type.id()==ID_unsignedbv)
  {
    ...
    if(a.over_state==0)
    {
      ...
      this->bv_utils.set_equal(r, a.result_bv);
    }
    else
      UNREACHABLE;
  }
  ...
  this->log.status() << "Found spurious '" << a.as_string() << "' (state "
...
/// underapproximation
template <typename bv_pointers_baset>
void bv_refinementt<bv_pointers_baset>::check_UNSAT(approximationt &a)
{
  // part of the conflict?
  if(!this->conflicts_with(a))
    return;

  this->log.status() << "Found assumption for '" << a.as_string()
                     << "' in proof (state " << a.under_state << ")"
                     << messaget::eom;
...
template <typename bv_pointers_baset>
bool bv_refinementt<bv_pointers_baset>::conflicts_with(approximationt &a)
{
  for(std::size_t i=0; i<a.under_assumptions.size(); i++)
  {
    if(this->prop.is_in_conflict(
...
template <typename bv_pointers_baset>
void bv_refinementt<bv_pointers_baset>::initialize(approximationt &a)
{
...
template <typename bv_pointers_baset>
typename bv_refinementt<bv_pointers_baset>::approximationt &
bv_refinementt<bv_pointers_baset>::add_approximation(const exprt &expr, bvt &bv)
{
  this->approximations.push_back(approximationt(approximations.size()));
  approximationt &a=approximations.back();

  std::size_t width = this->boolbv_width(expr.type());
  PRECONDITION(width!=0);

  a.expr=expr;
  a.result_bv = this->prop.new_variables(width);
  a.no_operands=expr.operands().size();
  this->set_frozen(a.result_bv);

  if(a.no_operands==1)
  {
    a.op0_bv = this->convert_bv(to_unary_expr(expr).op());
    this->set_frozen(a.op0_bv);
  }
  else if(a.no_operands==2)
  {
    a.op0_bv = this->convert_bv(to_binary_expr(expr).op0());
    a.op1_bv = this->convert_bv(to_binary_expr(expr).op1());
    this->set_frozen(a.op0_bv);
    this->set_frozen(a.op1_bv);
  }
  else if(a.no_operands==3)
  {
    a.op0_bv = this->convert_bv(to_multi_ary_expr(expr).op0());
    a.op1_bv = this->convert_bv(to_multi_ary_expr(expr).op1());
    a.op2_bv = this->convert_bv(to_multi_ary_expr(expr).op2());
    this->set_frozen(a.op0_bv);
    this->set_frozen(a.op1_bv);
    this->set_frozen(a.op2_bv);
  }
  else
    UNREACHABLE;
...
template <typename bv_pointers_baset>
std::string bv_refinementt<bv_pointers_baset>::approximationt::as_string() const
{
