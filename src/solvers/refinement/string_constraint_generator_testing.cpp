  ...
  const string_exprt &prefix, const string_exprt &str, const exprt &offset)
{
  ...
  a2.premise=isprefix;
  a2.body=equal_exprt(
    str[plus_exprt_with_overflow_check(qvar, offset)], prefix[qvar]);
  ...
  const function_application_exprt &f)
{
  ...
  lemmas_.push_back(implies_exprt(is_empty, s0.axiom_for_has_length(0)));
  lemmas_.push_back(implies_exprt(s0.axiom_for_has_length(0), is_empty));
  ...
  const function_application_exprt &f)
{
  ...
  string_not_contains_constraintt a5(
    ...
    s0,
    s1);
  not_contains_constraints_.push_back(a5);
