...
public:
  ...
  void finish_eager_conversion() override
  {
    ...
    const bool refuted = try_algebraic_solve();
    ...
    //
    // Note: try_algebraic_solve() returns true iff it asserted false
    // on the propagator. The internal `algebraic_solved` flag means
    // "do not retry"; it is set to true even when refutation failed,
...
protected:
  ...
  // blasting cost is saved entirely. If it doesn't, we replay the queued
  // assertions through SUB::set_to in finish_eager_conversion before
  // falling through to the SAT solver. On by default; set
  // DISABLE_DEFER_BITBLAST=1 to opt out for ablation experiments.
