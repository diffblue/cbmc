...
public:
  ...
  void finish_eager_conversion() override
  {
    ...
    //
    // The replay must happen BEFORE functions.finish_eager_conversion()
    // (and before SUB::finish_eager_conversion(), which finalizes array
    // axioms): a term may occur only inside a deferred assertion (e.g. an
    // uninterpreted-function application m(zero_extend(x))), and its
    // congruence axioms are generated by functions.finish_eager_conversion
    // from the set of applications seen so far. Replaying after that call
    // would register the application too late, drop the congruence axioms,
    // and yield spurious SAT.
    //
