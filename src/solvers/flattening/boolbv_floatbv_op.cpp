  ...
  else if(expr.type().id() == ID_vector || expr.type().id() == ID_complex)
  {
    ...
    if(subtype.id()==ID_floatbv)
    {
      ...
      for(std::size_t i=0; i<size; i++)
      {
        ...
        lhs_sub_bv.assign(
          lhs_as_bv.begin() + static_cast<std::ptrdiff_t>(i * sub_width),
          lhs_as_bv.begin() + static_cast<std::ptrdiff_t>((i + 1) * sub_width));
        rhs_sub_bv.assign(
          rhs_as_bv.begin() + static_cast<std::ptrdiff_t>(i * sub_width),
          rhs_as_bv.begin() + static_cast<std::ptrdiff_t>((i + 1) * sub_width));
        ...
        std::copy(
          sub_result_bv.begin(),
          sub_result_bv.end(),
          result_bv.begin() + static_cast<std::ptrdiff_t>(i * sub_width));
      }
