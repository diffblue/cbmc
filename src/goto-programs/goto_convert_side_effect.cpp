  ...
  const irep_idt &mode)
{
  ...
  {
    auto &complex_type = to_complex_type(constant_type);
    exprt real = from_integer(1, complex_type.subtype());
    exprt imag = from_integer(0, complex_type.subtype());
    constant = complex_exprt(real, imag, complex_type);
  }
  else
