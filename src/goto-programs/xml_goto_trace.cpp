...
public:
  ...
  void visit(const goto_trace_stept &step) const override
  {
    if(source_location_ != previous_source_location_)
    {
      ...
      if(xml_location_.name != "")
      {
        xmlt &xml_location_only = dest_.new_element("location-only");
        ...
        xml_location_only.new_element().swap(xml_location_);
      }
    }
  }
  ...
  void visit(const trace_assertt &step) const override
  {
    if(!step.cond_value)
    {
      ...
      xmlt &xml_failure = dest_.new_element("failure");
      ...
      if(xml_location_.name != "")
        xml_failure.new_element().swap(xml_location_);
    }
  }

  void visit(const trace_assignmentt &step) const override
  {
    assignment_decl_impl(step);
  }
  ...
  void visit(const trace_outputt &step) const override
  {
    ...
    xmlt &xml_output = dest_.new_element("output");
    ...
    if(xml_location_.name != "")
      xml_output.new_element().swap(xml_location_);

    for(const auto &arg : step.io_args)
    {
      xml_output.new_element("value").data = from_expr(ns_, "", arg);
      xml_output.new_element("value_expression").new_element(xml(arg, ns_));
    }
  }

  void visit(const trace_inputt &step) const override
  {
    xmlt &xml_input = dest_.new_element("input");
    ...
    for(const auto &arg : step.io_args)
    {
      xml_input.new_element("value").data = from_expr(ns_, "", arg);
      xml_input.new_element("value_expression").new_element(xml(arg, ns_));
    }

    if(xml_location_.name != "")
      xml_input.new_element().swap(xml_location_);
  }

  void visit(const trace_function_callt &step) const override
  {
    function_call_return_impl(step, "function_call");
  }

  void visit(const trace_function_returnt &step) const override
  {
    function_call_return_impl(step, "function_return");
  }

private:
  void assignment_decl_impl(const goto_trace_stept &step) const
  {
    irep_idt identifier = step.lhs_object.get_identifier();
    xmlt &xml_assignment = dest_.new_element("assignment");

    if(xml_location_.name != "")
      xml_assignment.new_element().swap(xml_location_);
    ...
    if(step.lhs_object_value.is_not_nil())
      value_string = from_expr(ns_, identifier, step.lhs_object_value);

    if(step.full_lhs.is_not_nil())
      full_lhs_string = from_expr(ns_, identifier, step.full_lhs);

    if(step.full_lhs_value.is_not_nil())
      full_lhs_value_string = from_expr(ns_, identifier, step.full_lhs_value);

    if(step.lhs_object_value.type().is_not_nil())
      type_string = from_type(ns_, identifier, step.lhs_object_value.type());
    ...
    if(step.lhs_object_value.is_not_nil())
      xml_assignment.new_element("value_expression")
        .new_element(xml(step.lhs_object_value, ns_));
  }

  void function_call_return_impl(
    const goto_trace_stept &step,
    const std::string &tag) const
  {
    xmlt &xml_call_return = dest_.new_element(tag);
    ...
    if(xml_location_.name != "")
      xml_call_return.new_element().swap(xml_location_);
  }
  ...
  xmlt &dest)
{
  ...
  for(const auto &step : goto_trace.steps)
  {
    ...
    step->accept(
      convert_visitort{
        ns, source_location, previous_source_location, xml_location, dest});
