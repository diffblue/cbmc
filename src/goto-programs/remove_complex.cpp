...
void remove_complex(exprt &expr)
{
  ...
  if(expr.id()==ID_complex_real)
  {
    ...
    expr.id(ID_member);
    expr.set(ID_component_name, "real");
  }
  else if(expr.id()==ID_complex_imag)
  {
    assert(expr.operands().size()==1);
    expr.id(ID_member);
    expr.set(ID_component_name, "imag");
  }
  ...
  if(expr.type().id()==ID_complex)
  {
       ...
       expr.id()==ID_mult || expr.id()==ID_div)
    {
      ...
      struct_expr.op0()=
        binary_exprt(member_exprt(expr.op0(), "real", subtype), expr.id(),
                     member_exprt(expr.op1(), "real", subtype));

      struct_expr.op0().add_source_location()=expr.source_location();
      
      struct_expr.op1()=
        binary_exprt(member_exprt(expr.op0(), "imag", subtype), expr.id(),
                     member_exprt(expr.op1(), "imag", subtype));      
    ...
    else if(expr.id()==ID_unary_minus)
    {
      ...
      struct_expr.op0()=
        unary_minus_exprt(member_exprt(expr.op0(), "real", subtype));

      struct_expr.op0().add_source_location()=expr.source_location();
      
      struct_expr.op1()=
        unary_minus_exprt(member_exprt(expr.op0(), "imag", subtype));
...
void remove_complex(typet &type)
{
  ...
  else if(type.id()==ID_complex)
  {
    ...
    struct_type.components()[0].set_name("real");
    struct_type.components()[1].type()=type.subtype();
    struct_type.components()[1].set_name("imag");    
