...
public:
  ...
  void PT(const edget &e, std::set<event_idt> &edges);
  void CT(const edget &e, std::set<event_idt> &edges);
  void CT_not_powr(const edget &e, std::set<event_idt> &edges);
  void IT(const edget &e, std::set<event_idt> &edges);

  void const_graph_explore(
    event_grapht &graph, event_idt next, event_idt end,
    std::list<event_idt> &old_path);
  void graph_explore(event_grapht& graph, event_idt next, event_idt end,
    std::list<event_idt> &old_path, std::set<event_idt> &edges);
