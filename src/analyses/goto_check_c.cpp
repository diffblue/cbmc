...
void goto_check_ct::collect_allocations(
...
void goto_check_ct::mod_overflow_check(const mod_exprt &expr, const guardt &guard)
{
...
void goto_check_ct::conversion_check(
  const exprt &expr,
  const guardt &guard)
{
...
goto_check_ct::conditionst goto_check_ct::get_pointer_dereferenceable_conditions(
  const exprt &address,
  const exprt &size)
{
  auto conditions =
...
optionalt<goto_check_ct::conditiont> goto_check_ct::get_pointer_is_null_condition(
  const exprt &address,
  const exprt &size)
{
  PRECONDITION(local_bitvector_analysis);
