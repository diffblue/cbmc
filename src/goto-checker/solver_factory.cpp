...
#include <solvers/sat/remote_sat.h>
...
std::unique_ptr<solver_factoryt::solvert> solver_factoryt::get_solver()
{
  if(options.get_bool_option("dimacs"))
    return get_dimacs();
  else if(options.get_bool_option("remote-sat"))
    return get_remote_sat();
  if(
...
std::unique_ptr<solver_factoryt::solvert> solver_factoryt::get_remote_sat()
{
  no_beautification();
  no_incremental_check();

  auto prop = util_make_unique<remote_satt>(message_handler);
