...
#ifdef __CPROVER_JSA_DEFINE_TRANSFORMERS
{
  ...
  if (__CPROVER_jsa_null != prev)
  {
    ...
    __CPROVER_jsa_assume(__CPROVER_jsa__internal_get_list(heap, node_id) == __CPROVER_jsa__internal_get_list(heap, prev));
  }
  if (__CPROVER_jsa_null != next)
  {
    __CPROVER_jsa_assume(node_id == __CPROVER_jsa__internal_get_previous(heap, next));
    __CPROVER_jsa_assume(node_id < next);
    __CPROVER_jsa_assume(__CPROVER_jsa__internal_get_list(heap, node_id) == __CPROVER_jsa__internal_get_list(heap, next));
  }
}
#else
;
...
#ifdef __CPROVER_JSA_DEFINE_TRANSFORMERS
{
  if (__CPROVER_jsa__internal_is_concrete_node(lhs_node_id))
  {
    ...
    __CPROVER_jsa_assume(heap->concrete_nodes[lhs_node_id].next == rhs_node_id || heap->concrete_nodes[lhs_node_id].previous == rhs_node_id);
  }
  ...
  else
  {
    ...
    if (node.next == rhs_node_id)
    {
      ...
      __CPROVER_jsa_assume(lhs_index == __CPROVER_jsa__internal_get_max_index(heap, lhs_node_id));
    }
    else
    {
      __CPROVER_jsa_assume(node.previous == rhs_node_id);
      __CPROVER_jsa_assume(lhs_index == 0);
      __CPROVER_jsa_assume(rhs_index == __CPROVER_jsa__internal_get_max_index(heap, rhs_node_id));
    }
  }
}
#else
...
#ifdef __CPROVER_JSA_DEFINE_TRANSFORMERS
{
  ...
  __CPROVER_jsa_assume(__CPROVER_jsa_null == __CPROVER_jsa__internal_get_head_node(h, list));
}
#else
;
#endif
...
#ifdef __CPROVER_JSA_DEFINE_TRANSFORMERS
{
  ...
  for (__CPROVER_jsa_iterator_id_t it=0; it < __CPROVER_JSA_MAX_ITERATORS; ++it)
  {
    ...
    {
      __CPROVER_jsa_assume(list < h->list_count);
      __CPROVER_jsa__internal_assume_valid_iterator_linking(h, list, next_node, next_index);
      __CPROVER_jsa__internal_assume_valid_iterator_linking(h, list, prev_node, prev_index);
      if (__CPROVER_jsa_null != next_node && __CPROVER_jsa_null != prev_node)
        __CPROVER_jsa__internal_assume_is_neighbour(h, next_node, next_index, prev_node, prev_index);
    }
  }
  // Limit list sizes in counterexamples
  for (__CPROVER_jsa__internal_index_t listc=0; listc < __CPROVER_JSA_MAX_LISTS; ++listc)
  {
    __CPROVER_jsa__internal_index_t count=0;
    for (__CPROVER_jsa__internal_index_t cnodec=0; cnodec < __CPROVER_JSA_MAX_CONCRETE_NODES; ++cnodec)
      if (h->concrete_nodes[cnodec].list == listc) ++count;
    for (__CPROVER_jsa__internal_index_t anodec=0; anodec < __CPROVER_JSA_MAX_ABSTRACT_NODES; ++anodec)
      if (h->abstract_nodes[anodec].list == listc) ++count;
    ...
    const __CPROVER_jsa_pred_id_t pred_id)
{
  ...
  const __CPROVER_jsa__internal_index_t sz=__CPROVER_JSA_PREDICATE_SIZES[pred_id];
...
#define __CPROVER_jsa_execute_pred_result *__CPROVER_jsa_execute_pred_result_op_ptr
    ...
    case 1:
      ...
      break;
    case 2:
      __CPROVER_jsa_pred_opcode_first_2: __CPROVER_jsa_execute_pred_result=__CPROVER_jsa_mod(__CPROVER_jsa_execute_pred_op0, __CPROVER_jsa_execute_pred_op1);
      __CPROVER_jsa_pred_opcode_last_2: __CPROVER_jsa_execute_pred_result=__CPROVER_jsa_execute_pred_result;
      break;
    case 3:
    ...
    case 4:
      ...
      break;
    case 5:
      __CPROVER_jsa_pred_opcode_first_5: __CPROVER_jsa_execute_pred_result=__CPROVER_jsa_plus(__CPROVER_jsa_execute_pred_op0, __CPROVER_jsa_execute_pred_op1);
    ...
    const __CPROVER_jsa__internal_index_t id)
{
  ...
  __CPROVER_jsa_node_id_t it_node=__CPROVER_jsa__internal_get_head_node(heap, it_list);
  ...
  for (distance=0; distance < __CPROVER_JSA_MAX_NODES; ++distance)
    if (it_node == it_range_end || it_node == __CPROVER_jsa_null) break;
    else it_node=__CPROVER_jsa__internal_get_next(heap, it_node);
  ...
  if (__CPROVER_jsa_null == source)
  {
    ...
    {
      if (__CPROVER_jsa_null == node) { break; }
      else { node=__CPROVER_jsa__internal_get_next(heap, node); }
    }
    node=head_node;
    if (node_count > distance)
    {
      ...
      for (node_count=0; node_count < __CPROVER_JSA_MAX_NODES; ++node_count)
        if (node_count >= skip_distance) break;
        else node=__CPROVER_jsa__internal_get_next(heap, node);
    }
  }
  else
  ...
  for (__CPROVER_jsa__internal_index_t i=0; i < __CPROVER_JSA_MAX_NODES; ++i)
  {
    if (i >= distance || __CPROVER_jsa_null == node) break;
    ...
    const __CPROVER_jsa__internal_index_t query_size)
{
  ...
  for (__CPROVER_jsa__internal_index_t i=1; i < __CPROVER_JSA_MAX_QUERY_SIZE; ++i)
  {
    if (i >= query_size) break;
    const __CPROVER_jsa_query_instructiont instr=query[i];
    __CPROVER_jsa_query_opcode_0: __CPROVER_jsa_stream_op(heap, list, it, instr.op1, instr.op0, instr.opcode);
  }
}
