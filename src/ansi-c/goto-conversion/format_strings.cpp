...
void parse_flags(std::string::const_iterator &it, format_tokent &curtok)
{
  while(*it == '#' || *it == '0' || *it == '-' || *it == ' ' || *it == '+')
  {
    ...
    {
    case '#':
      curtok.flags.push_back(format_tokent::flag_typet::ALTERNATE);
      break;
    case '0':
      curtok.flags.push_back(format_tokent::flag_typet::ZERO_PAD);
      break;
    case '-':
      curtok.flags.push_back(format_tokent::flag_typet::LEFT_ADJUST);
      break;
    case ' ':
      curtok.flags.push_back(format_tokent::flag_typet::SIGNED_SPACE);
      break;
    case '+':
      curtok.flags.push_back(format_tokent::flag_typet::SIGN);
      break;
    default:
  ...
  format_tokent &curtok)
{
  ...
  case 'e':
  case 'E':
    curtok.type = format_tokent::token_typet::FLOAT;
    break;
  case 'f':
  case 'F':
    curtok.type = format_tokent::token_typet::FLOAT;
  ...
  case 'A':
    ...
    break;
  case 'c':
    curtok.type = format_tokent::token_typet::CHAR;
    break;
  case 's':
    curtok.type = format_tokent::token_typet::STRING;
    break;
  case 'p':
    curtok.type = format_tokent::token_typet::POINTER;
    break;
  case '%':
