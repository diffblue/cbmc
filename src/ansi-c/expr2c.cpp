...
static std::string clean_identifier(const irep_idt &id)
{
  ...
  if(c_pos != std::string::npos && dest.rfind("::") == c_pos)
    dest.erase(0, c_pos + 2);
  else if(c_pos != std::string::npos)
  {
    for(std::string::iterator it2 = dest.begin(); it2 != dest.end(); ++it2)
      if(*it2 == ':')
        *it2 = '$';
      else if(*it2 == '-')
        *it2 = '_';
  ...
  const std::string &declarator)
{
  ...
  else if(src.id() == ID_string)
  {
    ...
    src.id() == ID_natural || src.id() == ID_integer || src.id() == ID_rational)
  {
  ...
  else if(src.id() == ID_fixedbv)
  {
    ...
    return q + "__CPROVER_fixedbv[" + std::to_string(width) + "][" +
  ...
  else if(src.id() == ID_signedbv || src.id() == ID_unsignedbv)
  {
    // annotated?
  ...
  else if(src.id() == ID_union)
  {
    ...
    for(union_typet::componentst::const_iterator it =
          union_type.components().begin();
        it != union_type.components().end();
        it++)
    {
      dest += ' ';
      dest +=
        convert_rec(it->type(), c_qualifierst(), id2string(it->get_name()));
      dest += ';';
    }
  ...
  else if(src.id() == ID_pointer)
  {
    ...
    if(q != "" && (!declarator.empty() || subtype_followed.id() == ID_pointer))
      new_declarator += " " + q;
  ...
  {
    const union_tag_typet &union_tag_type = to_union_tag_type(src);
  ...
  else if(src.id() == ID_code)
  {
    ...
    if(non_ptr_type->id() == ID_code || non_ptr_type->id() == ID_array)
  ...
  else if(src.id() == ID_constructor || src.id() == ID_destructor)
  {
  ...
  bool inc_size_if_possible)
{
  ...
  return convert_rec(src.subtype(), qualifiers, declarator_str + array_suffix);
}

std::string
expr2ct::convert_typecast(const typecast_exprt &src, unsigned &precedence)
{
  ...
  if(to_type.id() == ID_c_bool && from_type.id() == ID_bool)
    return convert_with_precedence(src.op(), precedence);

  if(to_type.id() == ID_bool && from_type.id() == ID_c_bool)
    return convert_with_precedence(src.op(), precedence);
...
std::string expr2ct::convert_with(const exprt &src, unsigned precedence)
{
  ...
  for(size_t i = 1; i < src.operands().size(); i += 2)
  {
    ...
    {
      const irep_idt &component_name = src.operands()[i].get(ID_component_name);
...
std::string expr2ct::convert_let(const let_exprt &src, unsigned precedence)
{
  if(src.operands().size() < 3)
    return convert_norep(src, precedence);
...
std::string expr2ct::convert_update(const exprt &src, unsigned precedence)
{
  ...
  bool full_parentheses)
{
  ...
  bool use_parentheses0 = precedence > p0 ||
  ...
  bool use_parentheses1 = precedence > p1 ||
  ...
  unsigned precedence)
{
  ...
  std::string dest = symbol;
  if(precedence >= p || (!symbol.empty() && has_prefix(op, symbol)))
    dest += '(';
  dest += op;
  if(precedence >= p || (!symbol.empty() && has_prefix(op, symbol)))
    dest += ')';
...
std::string expr2ct::convert_allocate(const exprt &src, unsigned &precedence)
{
  ...
  if(src.type().id() == ID_pointer && src.type().subtype().id() != ID_empty)
  {
...
std::string expr2ct::convert_prob_coin(const exprt &src, unsigned &precedence)
{
  if(src.operands().size() == 1)
...
std::string expr2ct::convert_literal(const exprt &src, unsigned &precedence)
{
  return "L(" + src.get_string(ID_literal) + ")";
}

std::string
expr2ct::convert_prob_uniform(const exprt &src, unsigned &precedence)
{
  if(src.operands().size() == 1)
...
std::string expr2ct::convert_comma(const exprt &src, unsigned precedence)
{
  if(src.operands().size() != 2)
    return convert_norep(src, precedence);
...
std::string expr2ct::convert_complex(const exprt &src, unsigned precedence)
{
  if(
    src.operands().size() == 2 && src.op0().is_zero() &&
...
std::string
expr2ct::convert_pointer_arithmetic(const exprt &src, unsigned &precedence)
{
  if(src.operands().size() != 2)
    return convert_norep(src, precedence);
...
std::string
expr2ct::convert_pointer_difference(const exprt &src, unsigned &precedence)
{
  if(src.operands().size() != 2)
    return convert_norep(src, precedence);
...
std::string
expr2ct::convert_member(const member_exprt &src, unsigned precedence)
{
  if(src.operands().size() != 1)
    return convert_norep(src, precedence);
  ...
  if(src.op0().id() == ID_dereference && src.operands().size() == 1)
  {
...
std::string expr2ct::convert_norep(const exprt &src, unsigned &precedence)
{
  lispexprt lisp;
  irep2lisp(src, lisp);
  std::string dest = "irep(\"" + MetaString(lisp.expr2string()) + "\")";
...
std::string expr2ct::convert_symbol(const exprt &src, unsigned &precedence)
{
  const irep_idt &id = src.get(ID_identifier);
  std::string dest;

  if(
    src.operands().size() == 1 && src.op0().id() == ID_predicate_passive_symbol)
...
std::string
expr2ct::convert_predicate_symbol(const exprt &src, unsigned &precedence)
{
...
std::string
expr2ct::convert_predicate_next_symbol(const exprt &src, unsigned &precedence)
{
  const std::string &id = src.get_string(ID_identifier);
  return "pns(" + id + ")";
}
...
std::string
expr2ct::convert_quantified_symbol(const exprt &src, unsigned &precedence)
{
  const std::string &id = src.get_string(ID_identifier);
  return id;
}

std::string expr2ct::convert_nondet_bool(const exprt &src, unsigned &precedence)
{
  return "nondet_bool()";
}
...
std::string
expr2ct::convert_constant(const constant_exprt &src, unsigned &precedence)
{
    ...
    type.id() == ID_c_bit_field || type.id() == ID_c_bool)
  {
    ...
    const irep_idt &c_type = type.id() == ID_c_bit_field
                               ? type.subtype().get(ID_C_c_type)
    ...
    if(type.id() == ID_c_bool)
    {
      ...
      type == char_type() && type != signed_int_type() &&
    ...
    else
    {
      ...
      if(src.find(ID_C_c_sizeof_type).is_not_nil() && sizeof_nesting == 0)
      {
  ...
  else if(type.id() == ID_floatbv)
  {
    ...
    if(dest != "" && isdigit(dest[dest.size() - 1]))
    {
      ...
      if(src.type() == float_type())
        ...
        src.type() == long_double_type() && double_type() != long_double_type())
        dest += 'l';
    }
  ...
  else if(type.id() == ID_array || type.id() == ID_incomplete_array)
  {
...
std::string expr2ct::convert_vector(const exprt &src, unsigned &precedence)
{
  const typet full_type = ns.follow(src.type());
...
std::string expr2ct::convert_array(const exprt &src, unsigned &precedence)
{
  std::string dest;
...
std::string
expr2ct::convert_initializer_list(const exprt &src, unsigned &precedence)
{
  std::string dest;
...
std::string
expr2ct::convert_designated_initializer(const exprt &src, unsigned &precedence)
{
  if(src.operands().size() != 1)
  {
    unsigned precedence;
...
std::string expr2ct::convert_code_asm(const code_asmt &src, unsigned indent)
{
...
std::string expr2ct::convert_code_while(const code_whilet &src, unsigned indent)
{
  if(src.operands().size() != 2)
  {
    unsigned precedence;
  ...
  else
  {
    dest += ")\n";
    dest += convert_code(
      src.body(), src.body().get_statement() == ID_block ? indent : indent + 2);
  }
...
std::string
expr2ct::convert_code_dowhile(const code_dowhilet &src, unsigned indent)
{
  if(src.operands().size() != 2)
  {
    unsigned precedence;
...
std::string
expr2ct::convert_code_ifthenelse(const code_ifthenelset &src, unsigned indent)
{
  if(src.operands().size() != 3)
  {
    unsigned precedence;
...
{
  if(!src.operands().empty() && src.operands().size() != 1)
  {
    unsigned precedence;
    return convert_norep(src, precedence);
  }
...
std::string expr2ct::convert_code_break(const codet &src, unsigned indent)
{
  std::string dest = indent_str(indent);
  dest += "break";
  dest += ';';
...
std::string expr2ct::convert_code_continue(const codet &src, unsigned indent)
{
  std::string dest = indent_str(indent);
  dest += "continue";
  dest += ';';
...
std::string expr2ct::convert_code_decl(const codet &src, unsigned indent)
{
  ...
  if(!ns.lookup(to_symbol_expr(src.op0()).get_identifier(), symbol))
  {
    ...
    if(symbol->type.id() == ID_code && to_code_type(symbol->type).get_inlined())
      dest += "inline ";
  }
...
std::string expr2ct::convert_code_for(const code_fort &src, unsigned indent)
{
...
std::string expr2ct::convert_code_block(const code_blockt &src, unsigned indent)
{
  std::string dest = indent_str(indent);
  dest += "{\n";
...
std::string expr2ct::convert_code(const codet &src, unsigned indent)
{
  ...
  if(statement == "set_may" || statement == "set_must")
    return indent_str(indent) +
           convert_function(src, id2string(statement), 16) + ";";
...
std::string
expr2ct::convert_code_assign(const code_assignt &src, unsigned indent)
{
...
std::string expr2ct::convert_code_fence(const codet &src, unsigned indent)
{
  std::string dest = indent_str(indent) + "FENCE(";

  irep_idt att[] = {ID_WRfence,
                    ID_RRfence,
                    ID_RWfence,
                    ID_WWfence,
                    ID_RRcumul,
...
std::string expr2ct::convert_code_input(const codet &src, unsigned indent)
{
...
std::string expr2ct::convert_code_output(const codet &src, unsigned indent)
{
...
std::string expr2ct::convert_code_array_set(const codet &src, unsigned indent)
{
...
std::string expr2ct::convert_code_array_copy(const codet &src, unsigned indent)
{
...
std::string
expr2ct::convert_code_array_replace(const codet &src, unsigned indent)
{
...
expr2ct::convert_code_switch_case(const code_switch_caset &src, unsigned indent)
{
  ...
  if(
    src.code().get_statement() != ID_block &&
...
std::string expr2ct::convert_Hoare(const exprt &src)
{
  ...
  const codet &code = static_cast<const codet &>(src.find(ID_code));
...
std::string expr2ct::convert_extractbits(const exprt &src, unsigned precedence)
{
  if(src.operands().size() != 3)
    return convert_norep(src, precedence);
...
std::string expr2ct::convert_sizeof(const exprt &src, unsigned &precedence)
{
  if(src.has_operands())
    return convert_norep(src, precedence);
...
std::string
expr2ct::convert_with_precedence(const exprt &src, unsigned &precedence)
{
  precedence = 16;
  ...
  {
    if(src.operands().size() == 1 && src.op0().type().id() == ID_floatbv)
  ...
  else if(src.id() == ID_popcount)
  {
    ...
    src.id() == ID_integer_address || src.id() == ID_integer_address_object ||
    src.id() == ID_stack_object || src.id() == ID_static_object)
  {
  ...
  else if(src.id() == ID_address_of)
  {
    if(src.operands().size() != 1)
      ...
      src.op0().id() == ID_index && to_index_expr(src.op0()).index().is_zero())
  ...
  else if(src.id() == ID_initializer_list || src.id() == ID_compound_literal)
...
/// This methods converts a struct typet into a c struct declaration.
/// \param src The src typet, that should be converted.
/// \param qualifiers_str The qualifier of the struct
/// \param declarator_str The declarator that should be used for the struct
