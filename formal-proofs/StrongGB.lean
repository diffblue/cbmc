...
/-! ## F4-style tail reduction soundness

    The C++ implementation in `groebner.cpp::full_reduce` reduces
    EVERY term of a polynomial against the basis (not just the
    leading one). The C++ `interreduce_basis` applies this to a
    fixed point across the basis. Both are wired into the outer
    "Buchberger then interreduce then restart" loop.

    Soundness is by composition of the standard reduction lemma
    (`reduce_in_ideal` from BuchbergerCorrectness.lean): each tail
    reduction step `r := r - q * g` is a single `reduce_in_ideal`
    application; the lemma applies regardless of which term we
    choose to reduce.

    The theorem below states this orthogonality at the abstract
    level: tail reduction (a sequence of `r := r - q*g` steps where
    `q*g` is chosen to cancel a non-leading term) preserves the
    ideal generated by the basis. -/

/-- Single tail-reduction step: subtracting a multiple of a basis
    element from a polynomial preserves ideal membership. This is
    a direct consequence of `Ideal.span` being closed under
    subtraction and multiplication by ring elements; phrased here
    in the abstract form the C++ tail-reduction step takes. -/
theorem tail_reduction_in_ideal {n d : ℕ} (q : MvPolynomial (Fin n) (ZMod (2 ^ d)))
    (g r : MvPolynomial (Fin n) (ZMod (2 ^ d)))
    (basis : Set (MvPolynomial (Fin n) (ZMod (2 ^ d))))
    (h_g : g ∈ Ideal.span basis) (h_r : r ∈ Ideal.span basis) :
    (r - q * g) ∈ Ideal.span basis := by
  have h_qg : q * g ∈ Ideal.span basis := Ideal.mul_mem_left _ q h_g
  exact Ideal.sub_mem _ h_r h_qg

/-- F4-style interreduction soundness: applying tail reduction
    fixed-point across the basis preserves the ideal generated by
    the basis. The C++ `interreduce_basis` implements this. -/
theorem interreduce_preserves_ideal {n d : ℕ}
    (basis : Set (MvPolynomial (Fin n) (ZMod (2 ^ d))))
    (basis' : Set (MvPolynomial (Fin n) (ZMod (2 ^ d))))
    (h_subset : basis' ⊆ Ideal.span basis)
    (h_supset : basis ⊆ Ideal.span basis') :
    Ideal.span basis = Ideal.span basis' := by
  apply le_antisymm
  · exact Ideal.span_le.mpr h_supset
  · exact Ideal.span_le.mpr h_subset
