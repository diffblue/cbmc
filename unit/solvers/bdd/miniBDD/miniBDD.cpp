...
SCENARIO("miniBDD", "[core][solver][miniBDD]")
{
  ...
  GIVEN("A bdd for x&!x==0")
  {
    ...
    equal_exprt equality(
      bitand_exprt(var, bitnot_exprt(var)), from_integer(0, type));

    literalt result = boolbv.convert(equality);
  ...
  GIVEN("A bdd for x+x==1")
  {
    ...
    symbol_exprt var("x", type);
    equal_exprt equality(plus_exprt(var, var), from_integer(1, type));
  ...
  GIVEN("A bdd for x*y==y*x")
  {
    ...
    equal_exprt equality(mult_exprt(var_x, var_y), mult_exprt(var_y, var_x));
  ...
  GIVEN("A bdd for x*x==2")
  {
    ...
    symbol_exprt var_x("x", type);
    equal_exprt equality(mult_exprt(var_x, var_x), from_integer(2, type));
  ...
  GIVEN("A bdd for x*x==4")
  {
    ...
    unsignedbv_typet type(8);
    symbol_exprt var_x("x", type);
    equal_exprt equality(mult_exprt(var_x, var_x), from_integer(4, type));
